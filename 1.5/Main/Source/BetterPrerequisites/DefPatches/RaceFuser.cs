...
using Verse;
using static BigAndSmall.BodyDefFusion;
...
namespace BigAndSmall
{
    ...
    public class MergableBody
    {
        ...
        public bool makeRobotVersion = true;
        public bool makeRobotFusions = true;
        ...
        public bool Fuse { get => fuse; }// && !fuseAll; }
    ...
    public class BodyDefFusion : Def
    {
        ...
        public static List<BodyDefFusion> Instances => DefDatabase<BodyDefFusion>.AllDefs.ToList();
    ...
    {
        public static Dictionary<string, FusedBody> FusedBodies = [];
        public BodyDef generatedBody = null;
        public MergableBody[] mergableBodies = null;
        public ThingDef thing = null;
        public bool fake = false;

        public FusedBody(BodyDef generatedBody, params MergableBody[] mergableBodies)
        {
            this.generatedBody = generatedBody;
            this.mergableBodies = mergableBodies;
            FusedBodies[GetKey(mergableBodies.Select(x=>x.bodyDef).ToArray())] = this;
        }
        

        public MergableBody SourceBody => mergableBodies[0];

        private static string GetKey(BodyDef[] bodyDefs)
        {
            return string.Join("|", bodyDefs.OrderBy(x => x.defName));
        }

        public static FusedBody TryGetBody(params BodyDef[] bodyDefs)
        {
            if (FusedBodies.TryGetValue(GetKey(bodyDefs), out var body)) return body;
            return FusedBodies.TryGetValue(GetKey([.. GetSubstituted(bodyDefs)]), out var body2) ? body2 : null;
        }

        private static List<BodyDef> GetSubstituted(BodyDef[] bodyDefs)
        {
            var substitutedBodies = bodyDefs.ToList();
            var allSubs = BodyDefFusion.Substitutions;
        ...
        public static bool HasKey(params BodyDef[] bodyDefs)
        {
            return FusedBodies.ContainsKey(GetKey(bodyDefs));
        }
    }
    ...
    public static class RaceFuser
    {
        ...
        /// Merge body parts.
        /// </summary>
        public static void CreateMergedBodyTypes()
        {
            ...
            foreach (var bodyOne in bodyDefsToMerge.Where(x => x.canBeFusionOne))
            {
                ...
                !FusedBody.HasKey(bodyOne.bodyDef, x.bodyDef) &&
                bodyOne.Fuse && x.Fuse))
                {
                    ...
                    foreach (var fusionCombo in allFusionCombinations)
                    {
                        ...
                        if (FusedBody.HasKey(allBodyDef))
                        {
                            continue;
                        }
                        ...
                        BodyDef genBody = MakeBodyDef(bodyOne, [.. secondaryBodies]);
                        ClonePartsRecursive(null, bodyOne.bodyDef.corePart, genBody, bodyOne, []);
                        ...
                        new FusedBody(genBody, [bodyOne, .. secondaryBodies]);
                    }
                }
            }
        ...
        private static BodyDef MakeBodyDef(MergableBody bodyOne, params MergableBody[] bodyTwoArray)
        {
            var defNameOne = bodyOne.overrideDefNamer ?? bodyOne.bodyDef.defName;
            ...
            {
                defName = $"G_{defNameOne}{string.Join("", secondaryDefNames)}",
                description = bodyOne.bodyDef.description,
                label = $"{bodyOne.prefixLabel} {secondarySuffixLabels}"
            };
            return genBody;
        }
        ...
        private static BodyPartRecord ClonePartsRecursive(BodyPartRecord genPartParent, BodyPartRecord source, BodyDef genBody, MergableBody bodyOne, List<BodyPartRecord> unTransfereredParts)
        {
            ...
            var nGenPart = new BodyPartRecord
            {
                ...
                visibleHediffRots = source.visibleHediffRots,
            };
            //Log.Message($"{genPartParent?.LabelCap}->{nGenPart?.LabelCap}, " +
            ...
            foreach (var child in source.parts)
            {
                ClonePartsRecursive(nGenPart, child, genBody, bodyOne, unTransfereredParts);
            }
            return nGenPart;
        }
        ...
        private static void MergeRecursively(BodyPartRecord genPart, BodyPartRecord partTwo, List<BodyPartRecord> unTransfereredParts, MergableBody mergeOne)
        {
            ...
            foreach (var part in partTwoParts)
            {
                if (unTransfereredParts.Contains(part))
                {
                    ...
                    var newPart = ClonePartsRecursive(genPart, part, genPart.body, mergeOne, unTransfereredParts);
