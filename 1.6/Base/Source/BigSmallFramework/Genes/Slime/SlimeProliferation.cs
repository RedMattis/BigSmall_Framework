...
public class CompAbilityEffect_SlimeProliferation : CompAbilityEffect
{
    ...
    public override bool Valid(LocalTargetInfo target, bool throwMessages = true)
    {
        ...
        if (parent.pawn.IsActuallyAdult() == false)
        {
            if (throwMessages)
            {
                Messages.Message("BS_UserTooYoung".Translate(otherPawn.Label),
                    otherPawn, MessageTypeDefOf.RejectInput, historical: false);
            }
            return false;
        }
        if (otherPawn.IsActuallyAdult() == false)
        {
            if (throwMessages)
            {
                Messages.Message("BS_TargetTooYoung".Translate(otherPawn.Label),
                    otherPawn, MessageTypeDefOf.RejectInput, historical: false);
            }
            return false;
        }
    ...
    public static void DoProliferate(Pawn parentA, Pawn parentB)
    {
        ...
        else if (parentB == null || parentB == parentA)
        {
            if (parentA.genes.CustomXenotype != null)
            {
                List<Gene> parGenes = [.. parentA.genes.Endogenes];
                foreach(var gene in parentA.genes.Endogenes)
                {
                    babyPawn.genes.AddGene(gene.def, false);
                }
        ...
        if (PawnUtility.TrySpawnHatchedOrBornPawn(babyPawn, parentA))
        {
            ...
            {
                babyPawn.playerSettings.AreaRestrictionInPawnCurrentMap = parentA.playerSettings.AreaRestrictionInPawnCurrentMap;
            }
