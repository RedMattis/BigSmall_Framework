    ...
    public class EngulfHediff : HediffWithComps, IThingHolder
    {
        ...
        public override void Tick()
        {
            ...
            if (Find.TickManager.TicksGame % tickRate == 0)
            {
                ...
                bool hasDigestionCap = pawn.health.capacities.CapableOf(PawnCapacityDefOf.Metabolism);
                if (hasDigestionCap)
                {
                    // Get pawn metabolism capacity
                    digestionEffiency = pawn.health.capacities.GetLevel(PawnCapacityDefOf.Metabolism);
